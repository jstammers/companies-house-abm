.PHONY: verify fix lint format format-check type-check install test test-cov test-integration test-uk-data test-uk-data-integration test-matrix test-matrix-cov pysentry docs docs-serve build-rust benchmark
...
test:
	uv run pytest tests/ packages/uk-data/tests/ -m "not integration" -v
...
test-integration:
	uv run pytest tests/ packages/uk-data/tests/ -m integration -v

# Run uk-data unit tests only
test-uk-data:
	uv run pytest packages/uk-data/tests/ -m "not integration" -v

# Run uk-data integration tests only
test-uk-data-integration:
