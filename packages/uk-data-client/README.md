...
`uk-data-client` wraps six data sources behind a single `UKDataClient` API,
...
└── tests/
    ├── test_uk_data_client.py               # Offline unit tests (~80 tests)
    ├── test_uk_data_client_integration.py   # Live network integration tests
    └── test_uk_data_client_property_sources.py  # Property/EPC source tests
```
...
make test-uk-data-client

# Integration tests (requires live internet access)
make test-uk-data-client-integration
...
`make test-uk-data-client-integration` or
