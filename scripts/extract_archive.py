...
def process_zip(zipfile, PROJECT_PATH, EXTRACT_DIR, OUTPUT_DIR):
	...
	output_path = OUTPUT_DIR / f"combined_records_{zipfile.stem}.csv"
...
def main():
	...
	EXTRACT_DIR = PROJECT_PATH / "02_intermediate"
	OUTPUT_DIR = PROJECT_PATH / "data" / "03_primary"
	bootstrap_project(project_path=PROJECT_PATH.as_posix())
	num_workers = 6
	with multiprocessing.Pool(num_workers) as pool:
		pool.starmap(process_zip, [(zipfile, PROJECT_PATH, EXTRACT_DIR, OUTPUT_DIR) for zipfile in INPUT_PATH.glob("*.zip")])
