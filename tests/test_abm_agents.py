...
class TestFirm:
    ...
    def test_init_with_behavior(self):
        ...
        firm = Firm(_model(), behavior=behavior)
    ...
    def test_step_bankrupt_firm_is_noop(self):
        firm = Firm(_model(), employees=5, turnover=100.0, capital=1000.0)
        firm.bankrupt = True
    ...
    def test_hire(self):
        firm = Firm(_model(), employees=5, wage_bill=50_000.0)
        firm.vacancies = 3
    ...
    def test_fire(self):
        firm = Firm(_model(), employees=10, wage_bill=100_000.0)
    ...
    def test_get_state_keys(self):
        ...
        assert isinstance(state["unique_id"], int)
        assert state["agent_type"] == "firm"
...
class TestHousehold:
    ...
    def test_step_unemployed(self):
        hh = Household(_model(), wealth=1000.0, mpc=0.8)
        hh.step()
    ...
    def test_become_unemployed(self):
        hh = Household(_model(), employed=True, employer_id="f1", wage=1000.0)
    ...
    def test_is_searching_unemployed_deterministic(self):
        hh = Household(_model(), employed=False)
        assert hh.is_searching() is True

    def test_is_searching_employed(self):
        hh = Household(_model(), employed=True)
    ...
    def test_consumption_cannot_exceed_resources(self):
        hh = Household(_model(), income=100.0, wealth=50.0, mpc=0.99)
        hh.employed = True
        hh.wage = 100.0
        hh.step()
    ...
    def test_transfer_income(self):
        hh = Household(_model(), employed=False, wealth=0.0)
...
class TestGovernment:
    ...
    def test_pay_unemployment_benefit(self):
        ...
        gov = Government(_model(), transfers=transfers)
