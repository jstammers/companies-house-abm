...
Agent-based models (ABMs) offer a uniquely powerful approach to simulating UK housing market dynamics because they capture the heterogeneity, feedback loops, and nonlinear amplification mechanisms that traditional econometric and DSGE models structurally miss. A decade of Bank of England-funded research — from Baptista et al. (2016) [1] through Carro et al. (2022) [2] to Bardoscia et al. (2024) [3] — has produced increasingly sophisticated, open-source, UK-calibrated housing ABMs that model households, banks, and policy interventions at the individual level. In parallel, the Washington DC housing bubble model by Geanakoplos, Axtell, and Farmer demonstrated that ABMs can reproduce observed boom-bust dynamics with remarkable fidelity at 1:1 population scale [4][5].
...
**Baptista et al. (2016), BoE Staff Working Paper No. 619** [1]:
...
**Carro et al. (2022), BoE Staff Working Paper No. 976** [2]:
...
**Bardoscia, Carro et al. (2024), BoE Staff Working Paper No. 1,066** [3]:
...
- `INET-Complexity/housing-model` (47 stars, Java) [6]
- `adrian-carro/spatial-housing-model` (13 stars, Java) [7]

### 1.2 The Washington DC Model (Geanakoplos, Axtell, Farmer et al.)

The INET-funded Washington DC housing bubble model is the most ambitious housing ABM to date [4][5][8]:
...
- Published in the American Economic Review (Geanakoplos et al. 2012) [4]

### 1.3 Other Notable Housing ABMs

| Model | Geography | Key Feature |
|-------|-----------|-------------|
| Gilbert, Hawksworth & Swinney (2009) [9] | UK | Early spatial housing ABM (50×50 grid), reproduced sticky-downward prices |
| Ge (2013) [10] | UK | Spatial model arguing lenient lending causes price bubbles |
| Laliotis et al. (2019), ECB WP 2294 [11] | European countries | Multi-country LTV caps assessment, calibrated per country |
| JASSS Korean housing ABM (2020) [12] | South Korea | Brochure-concept market mechanism (information asymmetry), LTV/DTI experiments |
...
The BoE models implement mortgage origination through a bank agent that [1][2][3]:
...
Separate UK mortgage delinquency models confirm the importance of the negative equity channel as a driver of both arrears and repossessions [14].
...
This amplification mechanism was central to the 2008 crisis and is explicitly captured in the Geanakoplos leverage cycle framework [15][16]. Traditional models estimate reduced-form relationships; ABMs generate this dynamic endogenously.
...
|---------|-----------|----------|
| **Interest rates** | Base rate → mortgage rate → affordability → demand → prices | Baptista (2016) [1], Geanakoplos (2012) [4] |
| **Unemployment** | Job loss → income shock → default + demand reduction | BoE WP 812 [13] |
| **Credit conditions** | LTV/LTI limits → credit availability → purchasing power → prices | Carro (2022) [2], Bardoscia (2024) [3] |
| **Bank capital** | Capital requirements → lending capacity → credit supply | Bardoscia (2024) [3] |
| **Population/migration** | Household formation → demand pressure | Not yet in published BoE ABMs |
| **Housing supply** | Planning permissions → new builds → supply | Not yet in published BoE ABMs |
| **Government policy** | Stamp duty, Help to Buy, Right to Buy → demand shifts | Not yet in published BoE ABMs |
| **Inflation** | Real income erosion, construction costs → affordability | Partially in Bardoscia (2024) [3] |
...
|--------|---------|-------------|--------|
| **HM Land Registry Price Paid Data** [17] | All property sales in England & Wales | Individual transactions (address, price, date, type) | Open Government Licence, monthly updates |
| **ONS UK House Price Index** [18][19] | Mix-adjusted house price index | Monthly, 441+ areas (LA/region) | Open, hedonic regression methodology |
...
|--------|---------|--------|
| **FCA Product Sales Data (PSD001/PSD007)** [20][21] | Loan-level mortgage origination data (amount, LTV, income, rate type, product type) | FCA regulated firms only; aggregate statistics published |
...
The BoE models use both open data and regulatory data (FCA PSD) [2]. A commercial product would need to work primarily with open data, supplemented by commercial data partnerships.

### 3.3 ONS HPI Methodology

The UK HPI uses a **hedonic regression** approach: property characteristics (type, size, age, location) are controlled for, so the index measures "like-for-like" price changes rather than being affected by compositional shifts in what is being sold [18]. The index is produced jointly by HM Land Registry, ONS, and Land and Property Services Northern Ireland. An ABM that generates synthetic transactions can compute a comparable mix-adjusted index for validation.
...
**Oxford Economics Real Estate Service** [22]:
...
**Moody's Analytics:**
- House Price Appreciation forecasts and mortgage portfolio stress testing tools
- Used extensively by US banks for CCAR/DFAST stress testing
- Statistical/econometric approach; UK coverage less granular than US
...
The PRA's 2025 stress testing framework requires banks to [25][26]:
- Project capital positions under prescribed adverse scenarios
- Stress mortgage portfolios by LTV band (benchmarks published for prime mortgages across LTV bands)
- Assess the impact of unemployment shocks, interest rate changes, and house price falls on mortgage defaults
- Banks not part of the concurrent stress test must use PRA-published scenarios

### 5.2 FPC Macroprudential Tools

The Financial Policy Committee has Powers of Direction over [27]:
...
Evaluating the impact of these tools before deployment is exactly what the BoE ABMs were built for [1][2][3].
...
Three main approaches dominate [28][29][30]:
...
2. **Moment matching**: price levels, transaction volumes, LTV distributions against empirical data
3. **Out-of-sample performance**: hindcast against known episodes (2008 GFC, COVID, 2022 rate rises)
...
Based on the JuliaDynamics/ABMFrameworksComparison (automated CI benchmarks) [31]:
...
| **FLAME GPU 2** | CUDA C++ / Python | Hundreds of millions of agents | GPU-accelerated [32] |
...
A full 1:1 simulation is feasible in Julia/Rust/C++ but would require careful memory management. The BoE models operate at reduced scale (representative sample). For a commercial product:
...
|-----------|---------------|-----------|
| **Core simulation** | Rust or Julia | Performance critical; Rust for memory safety + FFI; Julia for rapid prototyping |
...
The open-source BoE models (Java/MASON) [6][7] are valuable as reference implementations but have limitations for a commercial product:
- MASON's performance is 1-8× slower than Julia alternatives [31]
...
|--------|-----------|-------------|
| **Housing transaction market** | Double auction (following Baptista/Carro [1][2]) with spatial segmentation | House prices by region/type, transaction volumes, days on market |
| **Rental market** | Posted-price matching with search frictions | Rental yields, vacancy rates, rent levels |
| **Mortgage market** | Bank-mediated with affordability checks and regulatory constraints [3] | LTV distribution, mortgage volumes, interest rates, rejection rates |
| **Labour market** | Stochastic employment transitions calibrated to ONS data | Employment rate, income distribution |
...
| **Consultancies** (Big 4, boutique housing) | Client advisory, scenario analysis | High (reseller channel) |
...
5. **Regulatory acceptance**: Banks may not be able to use ABM outputs for regulatory capital unless validated against PRA expectations. Mitigation: position as complementary to existing models, not replacement
6. **Competition from BoE**: The BoE could develop its own production version. Mitigation: commercial product offers UX, speed, and customisation that a research tool does not
...
The academic foundations for a UK housing market ABM are strong and publicly available. The BoE/INET model lineage provides a validated, open-source starting point [1][2][3][6][7]. The gap between existing commercial tools (top-down econometric) and what an ABM offers (bottom-up, heterogeneous, nonlinear) is real and commercially meaningful, particularly for:
...
[1] Baptista, R., Farmer, J.D., Hinterschweiger, M., Low, K., Tang, D. & Uluc, A. (2016). "Macroprudential policy in an agent-based model of the UK housing market." BoE Staff Working Paper No. 619. https://www.bankofengland.co.uk/working-paper/2016/macroprudential-policy-in-an-agent-based-model-of-the-uk-housing-market

[2] Carro, A., Hinterschweiger, M., Uluc, A. & Farmer, J.D. (2022). "Heterogeneous effects and spillovers of macroprudential policy in an agent-based model of the UK housing market." BoE Staff Working Paper No. 976. https://www.bankofengland.co.uk/working-paper/2022/heterogeneous-effects-and-spillovers-of-macroprudential-policy-in-model-of-uk-housing-market

[3] Bardoscia, M., Carro, A., Hinterschweiger, M., Napoletano, M., Popoyan, L., Roventini, A. & Uluc, A. (2024). "The impact of prudential regulations on the UK housing market and economy: insights from an agent-based model." BoE Staff Working Paper No. 1,066. https://www.bankofengland.co.uk/working-paper/2024/the-impact-of-prudential-regulations-on-the-uk-housing-market-and-economy

[4] Geanakoplos, J., Axtell, R., Farmer, J.D., Howitt, P., Conlee, B., Goldstein, J., Hendrey, M., Palmer, N.M. & Yang, C.-Y. (2012). "Getting at Systemic Risk via an Agent-Based Model of the Housing Market." American Economic Review, 102(3), 53–58. https://www.aeaweb.org/articles.php?doi=10.1257%2Faer.102.3.53

[5] Axtell, R., Farmer, J.D., Geanakoplos, J. et al. (2024). "An Agent-Based Model of the Housing Market Bubble in Metropolitan Washington D.C." SSRN Working Paper. https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4710928

[6] INET-Complexity/housing-model. GitHub. https://github.com/INET-Complexity/housing-model

[7] adrian-carro/spatial-housing-model. GitHub. https://github.com/adrian-carro/spatial-housing-model

[8] "How to prevent the next housing bubble." INET Oxford. https://www.inet.ox.ac.uk/news/prevent-next-housing-bubble

[9] Gilbert, N., Hawksworth, J.C. & Swinney, P.A. (2009). "An agent-based model of the English housing market." AAAI Spring Symposium: Technosocial Predictive Analytics. https://cress.soc.surrey.ac.uk/housingmarket/index.html

[10] Ge, J. (2013). "Who creates housing bubbles? An agent-based study." International Workshop on Multi-Agent Systems and Agent-Based Simulation, Springer.

[11] Laliotis, D., Buesa, A., Leber, M. & Población García, F.J. (2019). "An agent-based model for the assessment of LTV caps." ECB Working Paper Series No. 2294.

[12] JASSS (2020). "Housing Market Agent-Based Simulation with Loan-To-Value and Debt-To-Income." Journal of Artificial Societies and Social Simulation, 23(4), 5. https://www.jasss.org/23/4/5.html

[13] BoE Staff Working Paper No. 812 (2019). "Three triggers? Negative equity, income shocks and institutions as determinants of mortgage default." https://www.bankofengland.co.uk/working-paper/2019/three-triggers-negative-equity-income-shocks-and-institutions-as-determinants-of-mortgage-default

[14] Aron, J. & Muellbauer, J. "Modelling and forecasting mortgage delinquency and foreclosure in the UK." Oxford University Research Archive. https://ora.ox.ac.uk/objects/uuid:f27f6e1c-3a1b-4501-b59d-cdb14d9e1b53

[15] Geanakoplos, J. (2010). "The Leverage Cycle." NBER Macroeconomics Annual 2009, Volume 24. https://www.journals.uchicago.edu/doi/full/10.1086/648285

[16] Geanakoplos, J. "Leverage Caused the 2007-2009 Crisis." Yale University. https://campuspress.yale.edu/johngeanakoplos/files/2019/11/Leverage-Caused-the-2007-2009-Crisis.pdf

[17] HM Land Registry. "Price Paid Data." https://www.gov.uk/government/statistical-data-sets/price-paid-data-downloads

[18] "Quality and methodology — UK House Price Index." GOV.UK. https://www.gov.uk/government/publications/about-the-uk-house-price-index/quality-and-methodology

[19] "About — UK House Price Index." HM Land Registry. https://landregistry.data.gov.uk/hpi-documentation.html

[20] FCA. "Interpreting the data — Product Sales Data." https://fca.org.uk/data/product-sales-data/interpreting-data

[21] FCA. "Mortgage sales data FAQs." https://www.fca.org.uk/firms/regdata/psd-new-mortgage-requirements-faqs/sales-data

[22] Oxford Economics. "Real Estate Forecasts and Scenarios." https://www.oxfordeconomics.com/service/consulting-services/bespoke-forecasting-and-scenarios/real-estate-consulting/

[23] Simudyne. "A Complete Guide to Agent-Based Modeling For Financial Services." https://simudyne.com/wp-content/uploads/2019/08/Guide-to-ABM-new-address.pdf

[24] Simudyne Docs. "Lending Model." https://docs.simudyne.com/2.5/financial_toolkit/balance_sheet

[25] Bank of England. "Stress testing the UK banking system: Guidance on the 2025 stress test." https://www.bankofengland.co.uk/stress-testing/2025/guidance-for-participants

[26] Regulation Tomorrow. "PRA publishes stress test scenarios for banks and building societies not part of concurrent stress testing." (April 2025). https://www.regulationtomorrow.com/2025/04/pra-publishes-stress-test-scenarios-for-banks-and-building-societies-not-part-of-concurrent-stress-testing/

[27] Bank of England. "The buy-to-let sector and financial stability." Quarterly Bulletin, 2023. https://bankofengland.co.uk/quarterly-bulletin/2023/2023/the-buy-to-let-sector-and-financial-stability

[28] Platt, D. (2019). "A Comparison of Economic Agent-Based Model Calibration Methods." arXiv:1902.05938. https://arxiv.org/pdf/1902.05938

[29] Lux, T. (2021). "Bayesian Estimation of Agent-Based Models via Adaptive Particle Markov Chain Monte Carlo." Computational Economics. https://link.springer.com/content/pdf/10.1007/s10614-021-10155-0.pdf

[30] Emergent Mind. "Calibrated Agent-Based Models." https://www.emergentmind.com/topics/calibrated-agent-based-model

[31] JuliaDynamics/ABMFrameworksComparison. GitHub. https://github.com/JuliaDynamics/ABMFrameworksComparison

[32] FLAME GPU 2. GitHub. https://github.com/FLAMEGPU/FLAMEGPU2/

[33] BoE (2026). "A UK-HANK model." Macro Technical Paper No. 7. https://www.bankofengland.co.uk/macro-technical-paper/2026/a-uk-hank-model

[34] SpringerLink (2025). "An Agent-Based Model for Housing Markets: Impact of a Financial Shock on Wealth in the UK." https://link.springer.com/chapter/10.1007/978-3-031-91782-0_10
